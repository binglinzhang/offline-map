...
(function($, undefined){
	...
	var ret=$.widget("ui.sliderDemo", {
		options:{
		},

		_title: "",
		_name: "dateLineSlider",
		_create: function(){
...
//			this._elements.log.scrollTop(this._elements.log[0].scrollHeight);
			sendSliderData(e.type,data.values.min,data.values.max);
...
function sendSliderData(type,min,max)
{
	if($('#minAndMax'))
	{
		$('#minAndMax').html('type='+type+'<br/>min='+min.getTime()+'<br/>max='+max.getTime());
	}	
}